...
export default function RejectedOrg({ orgName }: { orgName: string }) {
  const { t } = useLang()
  const [before, after] = t.orgRejectedBody.split('{org}')
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950">
      <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-10 w-full max-w-sm text-center shadow-2xl">
        <h1 className="text-xl font-bold text-slate-100 mb-3">{t.orgRejectedTitle}</h1>
        <p className="text-slate-400 text-sm mb-8 leading-relaxed">
          {before}<span className="font-medium text-slate-200">{orgName}</span>{after}
        </p>
        <button
